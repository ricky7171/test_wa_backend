...
func (s *subscription) writePump() {
	...
	for {
		select {
		case message, ok := <-c.send: //jika ada pesan masuk ke dalam room
			if !ok {
				c.write(websocket.CloseMessage, []byte{}) //write ke ws, tipe write nya adalah close message, isinya adalah {}
				return
			}
			if err := c.write(websocket.TextMessage, message); err != nil { //coba write ke ws, tipenya adalah textmessage, isinya adalah variabel message, kalau tidak error maka
				return
			}
